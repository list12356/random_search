...
# def compute_composition_stats(evaluator,
...
def evaluate_piece(evaluator,
    ...
    Returns:
        ...
        created.
    """
    evaluator.reset_composition()

    for i in range(composition_length):

        obs_note = piece[i]
        new_observation = np.eye(n_words)[obs_note]

        # Compute note by note stats as it composes.
        stat_dict = add_interval_stat(evaluator, new_observation, stat_dict, key=key)
        stat_dict = add_in_key_stat(obs_note, stat_dict, key=key)
        stat_dict = add_tonic_start_stat(
                evaluator, obs_note, stat_dict, tonic_note=tonic_note)
        stat_dict = add_repeating_note_stat(evaluator, obs_note, stat_dict)
        stat_dict = add_motif_stat(evaluator, new_observation, stat_dict)
        stat_dict = add_repeated_motif_stat(evaluator, new_observation, stat_dict)
        stat_dict = add_leap_stats(evaluator, new_observation, stat_dict)

        evaluator.composition.append(np.argmax(new_observation))
        evaluator.beat += 1
        last_observation = new_observation

    for lag in [1, 2, 3]:
        stat_dict['autocorrelation' + str(lag)].append(
                rl_tuner_ops.autocorrelate(evaluator.composition, lag))

    add_high_low_unique_stats(evaluator, stat_dict)
...
def add_interval_stat(evaluator, action, stat_dict, key=None):
    ...
    Returns:
        ...
        intervals.
    """
    interval, _, _ = evaluator.detect_sequential_interval(action, key)
...
def add_tonic_start_stat(evaluator,
                                                 action_note,
                                                 stat_dict,
                                                 tonic_note=rl_tuner_ops.C_MAJOR_TONIC):
    ...
    Returns:
        ...
        updated.
    """
    if evaluator.beat == 0 and action_note == tonic_note:
        stat_dict['num_starting_tonic'] += 1
    return stat_dict


def add_repeating_note_stat(evaluator, action_note, stat_dict):
    ...
    Returns:
        ...
        updated.
    """
    if evaluator.detect_repeating_notes(action_note):
        stat_dict['num_repeated_notes'] += 1
    return stat_dict


def add_motif_stat(evaluator, action, stat_dict):
    ...
    Returns:
        ...
        updated.
    """
    composition = evaluator.composition + [np.argmax(action)]
    motif, _ = evaluator.detect_last_motif(composition=composition)
    if motif is not None:
        stat_dict['notes_in_motif'] += 1
    return stat_dict


def add_repeated_motif_stat(evaluator, action, stat_dict):
    ...
    Returns:
        ...
        field updated.
    """
    is_repeated, _ = evaluator.detect_repeated_motif(action)
    if is_repeated:
        stat_dict['notes_in_repeated_motif'] += 1
    return stat_dict


def add_leap_stats(evaluator, action, stat_dict):
    ...
    """
    leap_outcome = evaluator.detect_leap_up_back(action)
...
def add_high_low_unique_stats(evaluator, stat_dict):
    """Updates stat dict if evaluator.composition has unique extrema notes.
    ...
    Returns:
        ...
        field updated.
    """
    if evaluator.detect_high_unique(evaluator.composition):
        stat_dict['num_high_unique'] += 1
    if evaluator.detect_low_unique(evaluator.composition):
