...
class Generator(object):
    ...
    def build_model(self, gradient=True):
        ...
        with tf.variable_scope("RNN") as vs_lstm:
            current_emb = tf.nn.embedding_lookup(self.Wemb, self.start_token) + self.bemb
            ...
            with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                for i in range(1, self.sequence_length): # maxlen + 1
                    ...
                    adv_loss = adv_loss + tf.reduce_mean(cross_entropy*self.reward[:, i])
        ...
        if gradient ==True:
            ...
            self.loss = adv_loss / self.sequence_length # / tf.reduce_sum(self.mask[:,1:])
    ...
    def build_generator(self, maxlen):
        ...
        for i in range(self.sequence_length - maxlen):
            self.generated_words.append(tf.ones([self.batch_size], dtype=tf.int32) * NOTE_OFF)
        # batch x seq_len
        self.generated_words = tf.transpose(tf.stack(self.generated_words))
    
    def build_generator_fix(self, maxlen):
        self.generated_words_fix = []
        state = self.lstm.zero_state(batch_size=self.batch_size, dtype=tf.float32)
        ...
        with tf.variable_scope("RNN"):
            ...
            with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                for i in range(maxlen):
                    ...
                    self.generated_words_fix.append(max_prob_word)

        for i in range(self.sequence_length - maxlen):
            self.generated_words_fix.append(tf.ones([self.batch_size], dtype=tf.int32) * NOTE_OFF)
        # batch x seq_len
        self.generated_words_fix = tf.transpose(tf.stack(self.generated_words_fix))
    ...
    def get_reward(self, sess, sentence, rollout_num, discriminator):
        rewards = []
        for i in range(rollout_num):
            for given_num in range(1, len(sentence[0])): #sequence_length
                feed = {self.sentence: sentence, self.given_num: given_num}
                samples = sess.run(self.generated_words_2, feed_dict=feed)
                feed = {discriminator.sentence: samples}
            ...
            feed = {discriminator.sentence: sentence}
            ypred_for_auc = sess.run(discriminator.ypred_for_auc, feed)
            ypred = np.array([item[1] for item in ypred_for_auc])
            if i == 0:
                rewards.append(ypred)
