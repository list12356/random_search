...
class Generator:
    def __init__(self, Z_dim, data_dim, pac_num, mode):
        ...
        self.update_ph = [tf.placeholder(tf.float32, shape=size) for size in self.size]
        update_op_list = []
        for t in range(len(self.theta_G)):
            update_op_list.append(tf.assign(self.theta_G[t], self.theta_G[t] + self.update_ph[t]))
        self.update_op = tf.group(*update_op_list)
        # self._setfromflat = U.SetFromFlat(self.theta_G)
        # self._getflat = U.GetFlat(self.theta_G)
        self.build()
    ...
    def update(self, theta_list):
        sess = tf.get_default_session()
        feed = {}
        for t in range(len(self.theta_G)):
            feed[self.update_ph[t]] = theta_list[t]
        # import pdb; pdb.set_trace()
        sess.run(self.update_op, feed_dict=feed)
        # self._setfromflat(theta_list)
        return
