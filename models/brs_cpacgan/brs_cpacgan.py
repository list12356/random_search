...
class BRSCPacGAN:
    def __init__(self, out_dir = "out_brs_cpac", alpha=0.2, v=0.02, _lambda = 1.0, 
        ...
        self.optimizer = Adam(self.G, self.alpha)
        print("delta")
    ...
    def train(self):
        ...
        for it in range(self.train_num):
            ...
            if self.mode == "gradient":
                ...
                update_list = [None] * len(self.G.theta_G)
                for t in range(len(self.G.theta_G)):
                    update_list[t] = np.sum(reward * np.transpose(delta[t], \
                        list(range(1, len(self.G.size[t]) + 1)) + [0]), axis=-1) \
                        / (self.num_workers * self.v)
                self.optimizer.update(update_list)
            ...
            if it % 100 == 0:
                ...
                print('Iter:' + str(it) + '\t time:' + str(end - start))
