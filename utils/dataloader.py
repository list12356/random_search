...
class MusicDataLoader(DataLoader):
    def __init__(self, batch_size, seq_length, end_token=0, num_special_events=2):
        ...
        self.sequences = []
        self.melodies = []
        self.input_dir = "./data/nottingham/MIDI/melody/"
        self.pointer = 0
    ...
    def create_batches(self):
        ...
        for file in os.listdir(self.input_dir):
            if file.endswith("mid"):
                m = midi_file_to_melody(self.input_dir + file)
                self.melodies.append(m)
                tmp = []
                for x in m:
                    if len(tmp) < self.seq_length:
                        tmp.append(x)
                while len(tmp) < self.seq_length:
                    tmp.append(-2)
                self.sequences.append(tmp)
        self.num_batch = len(self.sequences) // self.batch_size
        self.sequences = self.sequences[:self.num_batch * self.batch_size]
        # encode the event list
        self.min_note = 127
        self.max_note = 0
        for line in self.sequences:
            for x in line:
                if x > 0:
                    self.min_note = min(self.min_note, x)
                    self.max_note = max(self.max_note, x)

        for i in range(len(self.sequences)):
            for j in range(self.seq_length):
                x = self.sequences[i][j]
                if x > 0:
                    x = x + self.num_special_events - self.min_note
                else:
                    x = x + self.num_special_events
                self.sequences[i][j] = x
        self.sequences = np.array(self.sequences)
        self.sequence_batch = np.split(self.sequences, self.num_batch, 0)
        print('Finished processing MIDI files, max note ' + str(self.max_note) + ', min note: ' + str(self.min_note))
        return self.max_note - self.min_note + self.num_special_events + 1, self.min_note
    ...
    def decoder(self, event_list):
        ...
        """
        return [e - self.num_special_events if e < self.num_special_events else
                e + self.min_note for e in event_list]
